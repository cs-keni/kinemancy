...
- [ ] Implement `Particle` dataclass and `ParticleSystem` (spawn, update, render) in `src/particles.py`
- [ ] Rendering via `pygame.surfarray.blit_array()` only — NO per-sprite SRCALPHA blits
  - Float32 physics arrays, uint8 cast at blit time
  - Particle alpha < 5% → remove particle (prevents near-black artifacts that corrupt OBS Chroma Key)
- [ ] **Fire mode**: upward particles, orange → yellow → white color (HSV interpolation), vy = -2 to -5, decay = 0.03/frame
- [ ] **Water mode**: gravity-affected droplets, deep blue → cyan (RGB lerp), splash detection at screen edge
- [ ] **Lightning mode**: Bezier arcs between fingertip pairs, white/blue tint, alpha fades over 3 frames
- [ ] **Cosmic mode**: long-lived slow-drift star trails, purple → teal → white, decay = 0.005/frame
- [ ] Spawn at all 5 fingertip landmark positions per mode
- [ ] Mode switching: cycle fire → water → lightning → cosmic on wave gesture (manual trigger for now)
- [ ] Performance test: 60fps stable at ≤5,000 active particles
- [ ] Test OBS Chroma Key integration at end of Phase C
  - Primary: black `#000000` LWA_COLORKEY. If OBS keying is inconsistent: switch to magenta `#FF00FF`
- [ ] **Latency profiling** (milestone): profile full pipeline frame-by-frame, report P50/P95/P99 across 1,000 frames
- [ ] **Tests** (add to `tests/test_particles.py`):
  - Alpha < 5% cutoff: particle with life=0.04 → `active` mask → False after `update()`
  - Mode transition: fire→water→lightning→cosmic cycles correctly
  - Max particle count: spawning beyond MAX cap doesn't grow arrays
...
- [ ] Record 10-second clip in Fire mode
- [ ] Verify all Phase D exit gate criteria (objective, not vibes):
  - Fire mode: 60fps sustained, 500+ simultaneous particles visible
  - Per-fingertip trail with easing fade over ≥500ms
  - Snap gesture: starburst explosion (200 radial particles) triggering
  - Clap gesture: shockwave ring expanding from screen center
  - Clip recorded at 1080p minimum (OBS or Windows Game Bar)
...
- [ ] Build `scripts/augment.py`: generates 20 augmented variants per source sample
  - Scale ±30%, rotate ±45°, translate ±10%, mirror (x-axis flip), jitter ±0.5% Gaussian
- [ ] Build data collection UI: gesture name + live feed + record button → saves `.npy` to `data/gestures/{gesture}/{timestamp}.npy`
- [ ] Finalize `src/feature_extractor.py`:
  - `extract_static(landmarks) -> np.ndarray` (63-dim, wrist-origin, MCP-scale)
  - `extract_sequence(frames) -> np.ndarray` (T×63 for LSTM)
- [ ] Collect ~11 source samples per static gesture × 9 gestures (~99 source samples)
  - "none" class: synthesize ~500 OOD samples from transitional frames (NOT live-collected)
  - If val_acc < 90% on any class → collect 20+ samples for that class, re-augment
- [ ] Run `augment.py` → 20x augmented training set
- [ ] Write `train_static.py`: MLP architecture, temporal 70/15/15 split, training loop, save model
...
- [ ] Replace bootstrap via config flag (`"classifier": "trained"` in `config/actions.json`)
- [ ] Populate "Trained MLP" column in benchmark table
- [ ] **Tests** (add to `tests/test_inference.py`):
  - Per-gesture cooldown: gesture fires → cooldown dict updated → second fire within 800ms blocked
  - Confidence threshold: prediction at 0.84 confidence → no event; at 0.85 → event fires
  - 3-consecutive-frames counter: resets on gesture change, fires on 3rd same-gesture frame
...
- [ ] Collect ~11 source samples per dynamic gesture × 7 gestures (~77 source samples)
- [ ] Run `augment.py` for dynamic sequences (includes time-warp ±20% for LSTM)
- [ ] Implement `extract_sequence()` two-hand variant: 126-dim input (both hands concatenated, missing = zeros)
- [ ] Write `train_dynamic.py`: LSTM architecture, temporal 70/15/15 split
  - Architecture: `LSTM(input=63 or 126, hidden=128, num_layers=2, dropout=0.3) → FC(128,64) → FC(64,N)`
  - Loss: CrossEntropyLoss, Optimizer: Adam lr=5e-4
  - **MLflow**: same tracking as train_static.py
- [ ] Add sliding-window inference in Thread 2 (30-frame deque)
  - On camera reconnect: flush the 30-frame deque before resuming inference
- [ ] Add per-gesture cooldown: `{gesture_label: last_fired_timestamp}` dict, 800ms default
- [ ] Confidence thresholding: fire only if confidence ≥ 0.85 for ≥3 consecutive frames
- [ ] Add rule-based snap fallback (velocity spike on landmark 12 if LSTM snap accuracy < 90%)
- [ ] Evaluate on held-out test set — target ≥90% accuracy
- [ ] Populate "Trained LSTM" column in benchmark table
- [ ] **Tests** (add to `tests/test_inference.py`):
  - LSTM deque flush: after reconnect sentinel, both left_deque and right_deque are empty
  - Two-hand extraction: `extract_sequence(left_frames, right_frames=None×30)` → T×126 with zeros in right 63 dims
